...
class AudioTranscriber:
    ...
    def _preprocess_audio(self, audio: AudioSegment) -> AudioSegment:
        ...
        if audio.frame_rate != self.OPTIMAL_SAMPLE_RATE:
            logger.debug(f"Resampling from {audio.frame_rate}Hz to {self.OPTIMAL_SAMPLE_RATE}Hz")
...
def main() -> int:
    ...
    parser.add_argument(
        "--language",
        type=str,
        help="Force specific language code (e.g., en, es, fr)"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    parser.add_argument(
        "--show-metadata",
        action="store_true",
        help="Display transcription metadata"
    )
